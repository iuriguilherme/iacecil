...
def run_web(*argv) -> None:
    ...
    The supervisor spawns this as one of its children. It used to be
    module-level code that ran on import and also started every
    connector; the connectors are their own unit now, so a crash here
    stops only the web app (R2, R6).

    Configuration crosses the process boundary as argv: this process
    reads `instance/` itself rather than receiving live objects.
    """
    argv = list(argv) or list(sys.argv)
    try:
    ...
    system locale is set and available""")
        ...
        try:
            _bots: object = import_module(f"instance._bots_{argv[2]}")
