...
try:
    ...
    from ..furhat_bot.chatgpt import gptparrot
...
{log_level}""")
    ...
    try:
        ...
        logger.error(f"""Can't set locale to {config.locale}, make \
sure your system locale is set and available""")
        logger.exception(e)
    ...
    try:
        ...
        try:
            bots = ['furhatgpt']
            modules: list = [import_module('config')]
            configs: dict = {
                'furhatgpt': \
                (getattr(module, 'BotConfig')() \
                if hasattr(module, 'BotConfig') \
            ...
            logger.exception(e)
            logger.error("""config.py file was not found, make sure it \
...
bots that were not configured: \
{[bot for bot in bots if not bot in configs]}\
""")
    
    asyncio.run(gptparrot(
        bots,
        configs['furhatgpt'].furhat,
        configs,
        configs['furhatgpt'].openai,
        skip_intro = configs['furhatgpt'].furhat.get('skip_intro',
          True),
        log_messages = configs['furhatgpt'].furhat.get('log_messages',
          False),
        add_startswith = configs['furhatgpt'].furhat.get(
          'add_startswith', None),
        add_endswith = configs['furhatgpt'].furhat.get('add_endswith',
          None),
    ))
