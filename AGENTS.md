...
This file provides guidance for AI Agents (like Hermes) when working in this repository. It focuses on agentic workflows, tool usage, and cross-session state management, and embeds the architectural map so agents that do not read CLAUDE.md still have full context. CLAUDE.md mirrors the same project facts.

## Agentic Workflow

### Tool Use Policy
- **Verify First:** Always use `read_file` or `search_files` before proposing changes. Never guess file contents.
- **Search Knowledge Store:** Check `docs/solutions/` for documented solutions to past problems (bugs, best practices, workflow patterns, or other institutional knowledge), organized by category with YAML frontmatter (module, tags, problem_type), before implementing or debugging in documented areas.
...
### Debugging
Use the `systematic-debugging` skill for root cause analysis before applying fixes.

## Quality Gates
- All new Python code must be compatible with Python 3.11.
